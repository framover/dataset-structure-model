...
- Twenty-one conformance cases `ebrains-*` describing datasets published on EBRAINS from the object listing of their bucket and their data descriptor. The report gains a second pass for EBRAINS datasets: how the bucket of a dataset version is found, where descriptors and buckets disagree, how the configs import into NANSEN, gaps G17–G22 (a published dataset has no root path; names that list several entities; month names; requirements that differ by variant; two constructs where the reader and the self-consistency suite disagree), and the result of running the MATLAB reader on all 44 cases.
