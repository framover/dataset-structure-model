...
- Conformance case `ebrains-fiorilli-2022`: an EBRAINS dataset described from the complete listing of its bucket and its data descriptor, with several files per session in one flat folder. The report gains a second pass for EBRAINS datasets, gap G17 (a published dataset has no root path), and the result of running the MATLAB reader on all 24 cases.
